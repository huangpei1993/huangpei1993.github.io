...
从这篇开始，我会用一个系列把手中的两个仓库——`micro-mall`（消费者商城）与 `micro-mall-admin`（运营管理后台）——完整拆解一遍。

爱购商城（AIGO）项目，业务和架构的拆解，这个项目是一个什么项目，分为消费者商城端（前台）和运营管理端（后台）；
...
| 支付平台 | 收银台、支付结果通知、支付查询 | 与交易环节对接的外部系统 |
| 平台自身 | 会员、商品、营销、搜索、购物车、交易 | 支撑两端的一批业务能力 |
