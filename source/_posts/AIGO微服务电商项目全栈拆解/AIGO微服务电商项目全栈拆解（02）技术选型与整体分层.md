...
## 三、两端前端：Vue 3 与 Vue 2.7 的"分裂"是刻意的
...
**为什么分裂而不是统一？** 关键在于**生态成本**：B 端的组合（Element UI + ECharts + 后台模板组件）沉淀在 Vue 2 时代，把这一整套迁移到 Vue 3 的收益有限、成本不低；而 C 端商城是从零搭建的用户端，没有历史包袱，直接采用 Vue 3 + Element Plus 的新生态。

部署上，两个前端最终都以**静态资源**的形式交付，由 Nginx 承载并转发对后端接口的请求——这也是前后端分离项目最常见的交付形态。

## 四、数据与中间件：选型与取舍

### 4.1 MySQL：领域库是边界，也是两套系统唯一的交汇点
...
检索（关键词 + 分类筛选）没有引入 Elasticsearch 这类独立搜索引擎，而是**由商品服务基于商品库直接承接**；独立的检索服务目前只是一个骨架，尚未承载实际检索流量。

这是演示项目规模下的取舍：省掉一套索引集群的部署与运维，代价是数据量增长后的检索性能。它属于"当前边界"，而不是"最终形态"——真实电商里，这一步的演进方向正是独立搜索引擎。
...
- **OpenTelemetry 作为采集标准**：两端用不同的传输方式接入（C 端走 OTLP gRPC、B 端走 OTLP HTTP），但语义统一——不会因为语言或框架不同而分裂出多套链路方案；
- **Grafana 作为统一入口**：指标、链路、日志三种数据都汇入 Grafana，排障时可以在同一个界面里从指标下钻到链路、再跳到日志；
- **遥测按环境开关**：本地开发默认关闭链路与指标上报，联调与部署环境才打开——避免开发流量污染监控数据。

## 六、整体分层：四层视角

把前面的所有组件按"请求怎么走"排一下，得到四层（命名沿用项目文档）：

| 层 | C 端 | B 端 |
| --- | --- | --- |
| 体验层 | Vue 3 商城用户端 | Vue 2.7 运营后台 |
...
这套"分层 + 公共库"的组合，是后续文章拆解工程结构时会反复见到的架构意图：**边界靠分层守住，共性靠收敛复用**。
...
1. 两侧是两条独立的接入链路：C 端是"HTTP → gRPC"的微服务链，B 端是一个 Gin 进程直连数据库；
2. 中部六个方块是 C 端的领域服务，order 在下单链路上同步调用其他四个领域服务（图中已标注）；
3. 底部的数据与平台层里，MySQL 是两套系统**唯一的交汇点**；
4. 注册中心、缓存、消息与可观测性同处这一层，不属于业务链路，但没有它们，服务无法协作与排障。
...
- 发布时可以逐个节点进行，保持整体可用；
- 三副本是"最小的高可用形态"——允许单节点故障，又不至于让资源开销翻倍太多。
...
把中间件集中到独立服务器，与业务节点解耦：业务节点可以自由扩缩容，中间件则是"共享底座"。需要注意的是，"最小化"的另一面是——**中间件保持单节点**：这套环境用应用层的三节点冗余来验证高可用形态，数据层与消息层的冗余留给更真实的环境去解决。
...
| C 端前端 | Vue 3.5 + Element Plus | 从零搭建，没有历史包袱 |
| B 端前端 | Vue 2.7 + Element UI + ECharts | 沿用后台模板生态，迁移成本高于收益 |
