...
这一篇进入整条链路上最重的一段——**订单与交易域**：从确认单、下单、支付、发货、收货，一直走到售后与发票。
...
| 辅助 | 订单操作历史表、订单到会员的事件 Outbox 表 | 后台操作留痕 |
...
6. **计算金额**：先算券的抵扣并写回对应明细，再算积分抵扣，最后汇总出订单的总额、券抵扣额、积分抵扣额与应付金额；
...
支付方式由「策略注册表」在服务启动时组装：每个策略向外暴露自己的编码、名称、提供方、交互形态（收银台 / 二维码）和支付方式编码。当前实际注册的有三个：
...
| 同步回跳 | 渠道把用户浏览器带回网关的支付回跳地址 | 不直接采信回跳参数，一律**以查单结果为准**；安全性完全由「必须查单」这一步兜住 |
| 异步通知 | 渠道服务器回调网关的通知地址 | **先验签，再查单**；只有「通知声明的状态」与「查单返回的状态、订单号、渠道流水、金额」全部一致才落库，最后才回 `success` 应答 |

三条路径最终都汇聚到**同一个状态同步事务**。这个事务做的事：
...
- **二维码页面**：二维码有有效期倒计时，到期自动刷新；用户支付后手动点击「检查支付结果」，前端**只发一次查询请求**（避免页面一打开就无条件打渠道）；
...
退款单本身还有一套独立的状态：未退款 / 处理中 / 成功 / 失败——它和售后状态是两个维度。
...
3. 底部是退款成功之后连带发生的三件事：完成售后单、按规则决定是否关闭原订单、调用会员服务做积分与成长值调整。
...
4. **金额从明细快照复制**，不采信客户端传来的金额：行实付金额、优惠券抵扣金额、积分抵扣金额、商品获得的积分与成长值、商品与 SKU 快照，全部从原订单明细拷贝到售后商品行；
...
**管理后台不直接改售后状态**，它只做三件事：查列表、查详情、发命令。命令通过一个专门的售后命令主题投递，一共有四种：
...
- **退款不归还库存**：已经出库的商品不能直接归还库存，需要管理人员手动添加库存；
...
- **邮箱**：当前只把「收件人、主题、附件名、正文」打印成一条格式化日志，**并没有真正发送邮件**。
