...
module.exports = class extends Component {
    render() {
        ...
        // 文章页仅保留正文和右侧目录，减少阅读时的非正文干扰。
        const hasPostTocOnRight = isPostPage && Array.isArray(config.widgets)
            && config.widgets.some(widget => widget.type === 'toc' && widget.position === 'right');
        const useThreeColumnLayout = !isPageOrPost && columnCount === 3;
        return <html lang={language ? language.substr(0, 2) : ''}>
            <Head site={site} config={config} helper={helper} page={page} />
            <body className={`is-${columnCount}-column has-navbar-fixed-top${hasPostTocOnRight ? ' is-post-two-column' : ''}`}>
                    ...
                    <div class="container">
                        <div class="columns">
                            <div class={classname({
                                ...
                                'column-main': true,
                                'is-12': columnCount === 1,
                                'is-8-tablet is-8-desktop is-6-widescreen': useThreeColumnLayout,
                                'is-8-tablet is-8-desktop is-9-widescreen': !useThreeColumnLayout && columnCount > 1
                            })} dangerouslySetInnerHTML={{__html: indexTopData + body}}></div>
                            {!isPostPage ? <Widgets site={site} config={config} helper={helper} page={page} position={'left'}/> : null}
                            {!isPageOrPost || hasPostTocOnRight ? <Widgets site={site} config={config} helper={helper} page={page} position={'right'}/> : null}
                        </div>
                    </div>
                </section>
