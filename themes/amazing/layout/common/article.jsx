...
// getIndexExcerpt 首页摘要优先使用作者标记的摘要；未标记时仅展示首段，保持列表页轻量可浏览。
function getIndexExcerpt(page) {
    const content = page.excerpt || page.content || '';
    const firstParagraph = content.match(/<p\b[^>]*>[\s\S]*?<\/p>/i);
    // 首页不展示内容中的装饰图片，正文详情页仍保留原始内容。
    return (firstParagraph ? firstParagraph[0] : content)
...
module.exports = class extends Component {
    render() {
        ...
        // 首页仅保留文章摘要，避免缩略图抢占阅读内容的视觉焦点。
        const cover = index ? null : (page.thumbnail ? page.thumbnail : null);
        const indexExcerpt = index ? getIndexExcerpt(page) : null;
    ...
    // 仅在已启用评论服务时显示评论数，避免关闭评论后仍出现示例数据。
    const showComment = comment !== undefined && comment.type !== undefined && (comment.type == 'gitalk' || comment.type == 'valine');
        ...
        return <Fragment>
            ...
            <div class="card">
                ...
                <article class={`card-content article${'direction' in page ? ' ' + page.direction : ''}`} role="article">
                    ...
                    {/* Content/Excerpt */}
                    <div class="content" dangerouslySetInnerHTML={{ __html: index ? indexExcerpt : page.content }}></div>
